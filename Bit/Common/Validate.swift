...
class Validate {
    static var sharedInstance: Validation {
        var validator = Validation()
        validator.minimumLength = 1
        validator.maximumLength = defaultMaxLength
        return validator
    }
    
    class func defaultText(text: String) -> Bool {
        return Validate.sharedInstance.validateString(text)
    }
